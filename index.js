...
const MAKE_WEBHOOK_URL = "https://hook.us2.make.com/6c88s86jeia5rxhu2yzr6tsuf8636l7k";     // URL to Make.com webhook
...
// When someone calls Twilio sends the webhook which we are handling here for api automation.
fastify.all('/incoming-call', async (request, reply) => {
    console.log('Incoming call');  // Log incoming call for debugging

    // Get all incoming call details from the request body or query string bcz twilio might send any of the two.
    const twilioParams = request.body || request.query;
    console.log('Twilio Inbound Details:', JSON.stringify(twilioParams, null, 2));  // Log call details being printed in console using json.
    ...
    let firstMessage = "Hello, welcome to Calley AI. How can I assist you today?";  // Demo first message. This one is the default message in case there was no past convo in our record.

// Make.com wehook will always receive only one type of request. If we are sending request with 5 variables to one route and 2 variables to another route it won't work. Just send 2 + 3(empty) variables to all routes.

    try {
        // Send a POST request to Make.com webhook to get a customized message for the caller. Payload to make.com should always contain route
        const webhookResponse = await fetch(MAKE_WEBHOOK_URL, {
            ...
            body: JSON.stringify({
                ...
                data1: callerNumber,  // Send caller's number, this will actually find about the past conversation with that number.
        ...
        if (webhookResponse.ok) {
            const responseText = await webhookResponse.text();  // Get the text response from the make.com webhook
            console.log('Make.com webhook response:', responseText);
            try {
                const responseData = JSON.parse(responseText);  // Try to parse the response from webhook as JSON
...
// WebSocket route to handle the media stream for real-time interaction. This is for the bidirectional communication over a call. Streaming the audio.
